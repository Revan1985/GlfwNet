...
namespace GlfwNet
{
	...
	PlatformHint Glfw::GetPlatform ()
	{
		return static_cast<PlatformHint>(glfwGetPlatform ());
	}

	bool Glfw::PlatformSupported (PlatformHint platform)
	{
		return glfwPlatformSupported (static_cast<int>(platform)) == GLFW_TRUE;
	}

	String^ Glfw::GetWindowTitle (Window^ window)
	{
	...
	/*void* Allocatefun (unsigned int size, void* user)
	{
		return Glfw::OnAllocate (size, IntPtr (user)).ToPointer ();
	}*/
